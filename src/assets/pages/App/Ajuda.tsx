...
export default function Ajuda() {
    ...
    const [contactForm, setContactForm] = useState({ emailRemetente: '', assunto: '', mensagem: '' });
    ...
    const handleContactSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setSendMsg('Mensagem enviada com sucesso! Entraremos em contacto brevemente pelo email fornecido.');
                setContactForm({ emailRemetente: '', assunto: '', mensagem: '' });
                setTimeout(() => setSendMsg(''), 7000);
            }
        } catch {
            setSendError('Erro de ligação. Verifica a tua ligação à internet e tenta novamente.');
        } finally {
            setIsSending(false);
        }
    };
                        ...
                        <form className="contact-form" onSubmit={handleContactSubmit}>
                            <div className="contact-group">
                                ...
                                <input
                                    ...
                                    placeholder="Ex: professor@escola.pt"
                                    required
                                    value={contactForm.emailRemetente}
                                    onChange={e => setContactForm({ ...contactForm, emailRemetente: e.target.value })}
                                />
                            </div>
                            <div className="contact-group">
                                <label>Assunto</label>
