...
export default function VerAluno() {
    ...
    const handleToggleArchive = async () => {
        ...
        const newEstado = String(aluno.estado) === '0' ? '1' : '0';
        try {
            const res = await fetch(`/api/alunos/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...form, estado: newEstado }),
            });
            if (!res.ok) throw new Error('Erro ao alterar o estado do aluno.');
            const updated = await res.json();
            setAluno(updated);
            toast.success(newEstado === '0' ? 'Aluno arquivado com sucesso!' : 'Aluno restaurado com sucesso!');
            navigate('/ver-todos-alunos');
            navigate('/ver-todos-alunos');
        } catch (err: any) {
            toast.error(err.message || 'Ocorreu um erro ao arquivar.');
        } finally {
            ...
            {professorId == aluno.professorId ? (
                <div className="ver-aluno-page">
                    ...
                    <div className="danger-zone-container" style={{ marginTop: '3rem', borderTop: '2px dashed #fca5a5', paddingTop: '2rem' }}>
                        ...
                        <div style={{ marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: '1px solid #fee2e2' }}>
                            <h3 style={{ color: '#991b1b', fontSize: '1.1rem', marginBottom: '0.5rem' }}>{String(aluno.estado) === '0' ? 'Restaurar' : 'Arquivar'} Aluno</h3>
                            <p style={{ color: '#666', marginBottom: '1rem', maxWidth: '600px', lineHeight: '1.5' }}>
                                {String(aluno.estado) === '0'
                                    ? 'Restaurar o aluno fará com que volte a aparecer na lista principal de alunos ativos.'
                                    : 'Arquivar o aluno remove-o da vista principal, mas não apaga o seu histórico ou dados. Podes restaurá-lo mais tarde.'}
                            </p>
                            <button className="btn-save" style={{ backgroundColor: '#f59e0b' }} onClick={handleToggleArchive} disabled={togglingArchive}>
                                {togglingArchive ? 'A processar...' : (String(aluno.estado) === '0' ? 'Restaurar Aluno' : 'Arquivar Aluno')}
                            </button>
                        </div>

                        <div>
                            <h3 style={{ color: '#991b1b', fontSize: '1.1rem', marginBottom: '0.5rem' }}>Eliminar Definitivamente</h3>
